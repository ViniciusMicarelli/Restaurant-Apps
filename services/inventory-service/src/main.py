"""Ponto de Entrada Principal do Microsserviço de Estoque, Ficha Técnica e Baixa Automática.

Controla insumos, fichas técnicas de produtos e movimentações de estoque
(entradas, perdas, devoluções, contagem). Consome o evento `order.created`
(Saga por coreografia, ADR-001) para dar baixa automática nos insumos.
"""
...
from src.presentation.api.v1.inventory_router import router as inventory_router
...
async def lifespan(_app: FastAPI) -> AsyncGenerator[None, None]:
    try:
        ...
        # `kitchen-service` — sem um nome de fila exclusivo por serviço, os
        # dois concorreriam pela mesma fila e só um receberia cada evento.
        await event_bus.subscribe(
            ORDER_CREATED_ROUTING_KEY, handler, queue_name="inventory-service.order.created"
        )
...
app = FastAPI(
    ...
    version="0.1.0",
    description="API de Estoque, Ficha Técnica de Produtos e Baixa Automática de Insumos.",
    docs_url="/docs",
    redoc_url="/redoc",
    lifespan=lifespan,
)
