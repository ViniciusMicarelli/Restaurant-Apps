...
interface OrderReadyCelebrationProps {
  message: string;
  onDone: () => void;
  durationMs?: number;
}
...
/**
 * Celebração (Rive) mostrada quando um pedido do cliente passa a `READY` —
 * mesmo asset (`rewards.riv`) usado no sucesso de pagamento do `admin-web` e
 * no `waiter-mobile`, aqui reaproveitado como "seu pedido está pronto!".
 * Fecha sozinha depois de `durationMs`.
 */
export const OrderReadyCelebration: React.FC<OrderReadyCelebrationProps> = ({
  message,
  onDone,
  durationMs = 2200,
}) => {
