 ...
 * um BR Code (EMV) válido. O `payment-service` não tem integração com
 * nenhum PSP: todo pagamento aqui é simulado/instantâneo, igual ao cartão. */
...
/** Tela de Pix — sem campos de cartão nem assinatura (não fazem sentido pra
 * um Pix real: é o app do banco do cliente que autentica a transação). */
export const PixPaymentPanel: React.FC<PixPaymentPanelProps> = ({
  totalAmount,
  isSubmitting,
  onConfirm,
  onCancel,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-3">
        ...
        >
          {isSubmitting ? 'Processando...' : 'Confirmar Recebimento do Pix'}
        </button>
      </div>
    </div>
  );
